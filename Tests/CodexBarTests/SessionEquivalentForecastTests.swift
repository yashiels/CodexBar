...
extension SessionEquivalentForecastTests {
    ...
    func `generic weekly ambiguity preserves both sides of prior pair history`() async {
        ...
        func snapshot(weeklyWindows: [(String, Double)], at date: Date) -> UsageSnapshot {
            UsageSnapshot(
                primary: session,
                secondary: nil,
                extraRateWindows: weeklyWindows.map { id, usedPercent in
                    NamedRateWindow(
                        id: id,
                        title: "Weekly",
                        window: RateWindow(
